...
func FiltersFromSpec(spec *job.OCR2OracleSpec) (filters []logpoller.Filter, err error) {
	...
	coordinator, err2 := ethkey.NewEIP55Address(cfg.VRFCoordinatorAddress)
	if err2 != nil {
	...
	return []logpoller.Filter{createLogFilter(beacon.Address(), coordinator.Address(), dkg.Address())}, nil
}
